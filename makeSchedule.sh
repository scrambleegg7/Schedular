...
echo "[makeSchedule.sh] insert GoogleSheet into MongoDB(cloud) " >> $LOGS
cd $SCHEDULE
echo "[makeSchedule.sh]  batch process to insert data  " >> $LOGS
#$PYTHONP $SCHEDULE/insertLGSchedularBatch.py >> $LOGS
$PYTHONP $SCHEDULE/insertMongoLGSchedularBatch.py >> $LOGS

echo "[makeSchedule.sh] build integrate2.csv (newly added data from RECEPTY) for GoogleSheet " >> $LOGS
...
echo "------------------------------------" >> $LOGS
echo "[makeSchedule.sh] Append RECEPTY new data on Google Sheet." >> $LOGS
cd $GOOGLE
